  ...
  class DecoupledTabs {
    ...
    resetElement(target) {
      ...
      if (target.textSplitter) {
        ...
        // If the element is currently visible (is-active), hide it during revert
        // to prevent the letter-spacing "tick" caused by switching from inline-block
        // char wrappers back to native text nodes
        ...
        if (isVisible) {
          // Force a synchronous reflow so the reverted DOM is fully laid out
          // before we make it visible again — prevents the browser from painting
      ...
      try {
        ...
        const tl = gsap.timeline({
          onComplete: () => {
            ...
            // IMPORTANT: Do NOT restore any styles or revert SplitText here.
            // Any DOM/style change while the tab is visible causes a layout shift
            // ("tick") because the browser reflows from animation state to natural state.
            // All cleanup (SplitText revert, style restoration) happens in resetElement()
            // which is called when this tab gets hidden during the next tab switch.
