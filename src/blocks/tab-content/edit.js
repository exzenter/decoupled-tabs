...
const TRANSITION_OPTIONS = [
	{
		label: __( 'Inherit from Tab Area', 'decoupled-tabs' ),
		value: 'inherit',
	},
	{ label: __( 'None', 'decoupled-tabs' ), value: 'none' },
	{ label: __( 'Fade', 'decoupled-tabs' ), value: 'fade' },
	{
		label: __( 'Slide Horizontal', 'decoupled-tabs' ),
		value: 'slide-horizontal',
	},
	{
		label: __( 'Slide Vertical', 'decoupled-tabs' ),
			...
			<InspectorControls>
				<PanelBody title={ __( 'Tab Settings', 'decoupled-tabs' ) }>
					<TextControl
						...
						help={ __(
							'Unique identifier for this tab. Use this ID in data-tab-target attribute on trigger elements.',
							'decoupled-tabs'
						) }
						value={ tabId }
						onChange={ ( value ) =>
				...
				<PanelBody
					title={ __( 'Transition Override', 'decoupled-tabs' ) }
					initialOpen={ false }
				>
					<ToggleControl
						label={ __(
							'Override Tab Area Transition',
							'decoupled-tabs'
						) }
						help={ __(
							'Use custom transition settings for this tab instead of inheriting from Tab Area.',
							'decoupled-tabs'
						) }
						checked={ overrideTransition }
						onChange={ ( value ) =>
							setAttributes( { overrideTransition: value } )
						}
					/>
					{ overrideTransition && (
						<>
							<SelectControl
								...
								options={ TRANSITION_OPTIONS.filter(
